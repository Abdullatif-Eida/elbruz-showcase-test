...
class FormValidators {
  final List<FormFieldValidator<String>> _validators = List.empty(growable: true);
  ...
  FormValidators email({String? errorMessage}) {
    ...
    return maxLength(40, errorMessage: 'Please enter a valid email address').addValidator(emailValidator);
  }
  ...
  FormValidators maxLength(int n, {String? errorMessage}) {
    String? max(v) {
      if (v != null && v.length > n) {
        return errorMessage ?? 'Please enter a value less than $n characters long.';
      }
      return null;
    }
