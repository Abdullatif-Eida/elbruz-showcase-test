...
mixin StateMixin<T> on ChangeNotifier {
  ...
  Widget buildState({
    ...
    onLoading ??= (stateMessage) => const Center(child: CircularProgressIndicator.adaptive());
    ...
    if (viewStatus == ViewStatus.error) {
      return onError?.call(_stateMessage) ?? const Center(child: Text('خطا در دریافت اطلاعات'));
    }

    if (viewStatus == ViewStatus.initializing) {
      return onInitializing?.call(_stateMessage) ?? onLoading.call(_stateMessage);
    }
